...
def get_disparity_map(x, encoder, avg_frames=True):
    nbatch, nsources, nchannels, ntimes = x.shape
    X = encoder(x.contiguous().view(nbatch, nsources * nchannels, ntimes))[0] # n_frames, n_samples, n_channels, n_bins
    nframes, _, _, nbins = X.shape
    X = X.view(nframes, nbatch, nsources, nchannels, nbins)
    D = torch.stack([10*torch.log((X[:,:,s,0,:]**2 / X[:,:,s,1,:]**2)) for s in range(nsources)], 0) # nsources, nframes, nbatch, nbins
    
    mean = torch.mean(D, axis=-1) # nsources, nframes, nbatch
    std  = torch.std(D, axis=-1) # nsources, nframes, nbatch
    
    if avg_frames:
        mean = torch.mean(mean, axis=1, keepdim=True)
        std  = torch.mean(std, axis=1, keepdim=True)
...
class XUMXManager(System):
    ...
    def __init__(
        ...
        self.encoder = torch.nn.Sequential(_STFT(window_length=args.window_length, n_fft=args.in_chan, n_hop=args.nhop),_Spectrogram(spec_power=args.spec_power, mono=False))
    ...
    def common_step(self, batch, batch_nb, train=True):
        ...
        """
        inputs, targets, angles = batch
        if isinstance(self.model,XUMX_Spatial):
            # We encode the target's spatial information and pass it to the model
            mean_ild, std_ild = None, None#get_disparity_map(targets, self.encoder)
            est_targets = self(inputs, [mean_ild, std_ild, angles])
        else:
...
def main(conf, args):
    ...
    model = XUMX_Spatial if args.spatial else XUMX
    x_unmix = model(
